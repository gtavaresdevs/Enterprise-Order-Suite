...
@TestPropertySource(properties = {
  ...
  "security.rate-limit.refresh.capacity=3",
  "security.rate-limit.refresh.refill-seconds=10"
})
class RefreshRateLimitIT {
  ...
  @Test
  @DisplayName("Should rate limit refresh requests from the same IP after configured capacity is exceeded")
  void refresh_ipRateLimit_enforcesConfiguredCapacity() throws Exception {
    String ip = "10.10.10.99";
    String refreshToken = authenticateTestUser(ip);

    for (int attempt = 1; attempt <= RATE_LIMIT_CAPACITY; attempt++) {
      MvcResult result = performRefresh(ip, refreshToken);

      int status = result.getResponse().getStatus();

      assertThat(status)
        .as(
          "Refresh attempt %d should reach the refresh endpoint before the IP rate limit is exceeded",
          attempt
        )
        .isNotEqualTo(429);
    }

    MvcResult rateLimitedResult = performRefresh(ip, refreshToken);

    assertThat(rateLimitedResult.getResponse().getStatus())
      .as("The request exceeding the configured refresh capacity should be rate limited")
      .isEqualTo(429);
  ...
  private MvcResult performRefresh(String ip, String refreshToken) throws Exception {
  ...
  private void verifyRateLimitResponse(MvcResult result) throws Exception {
    assertThat(result.getResponse().getHeader("Retry-After"))
      .as("Rate-limited refresh responses must provide a Retry-After header")
      .isNotBlank();
  ...
  private String uniqueEmail() {
    return "refresh-ratelimit-" + UUID.randomUUID() + "@test.com";
  }
}
